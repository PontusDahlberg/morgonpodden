...
def generate_audio_google_cloud(segments: List[Dict], output_file: str) -> bool:
    ...
    try:
        ...
        if audio_file and os.path.exists(audio_file):
            ...
            logger.info(f"[TTS] Audio genererad med Google Cloud TTS: {output_file}")
            return True
        else:
            logger.error("[ERROR] Google Cloud TTS-generering misslyckades")
            return False
            
    except ImportError as e:
        logger.error(f"[ERROR] google_cloud_tts.py import error: {e}")
        return False
    except Exception as e:
        logger.error(f"[ERROR] Fel vid audio-generering: {e}")
        return False
